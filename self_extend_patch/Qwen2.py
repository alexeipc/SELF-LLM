...
def apply_grouped_rotary_pos_emb(q, k, cos, sin, position_ids, device, g_size_1=1, g_size_2=4096):
    ...
    position_ids_q -= position_ids_q[position_ids_q > 0].min()
    position_ids_k -= position_ids_k[position_ids_k > 0].min()

    position_ids_q = position_ids_q.clamp(0, cos.size(0) - 1)
    position_ids_k = position_ids_k.clamp(0, cos.size(0) - 1)
    
    # print(position_ids_q, position_ids_k)
...
def flash_self_extend_forward(
    ...
    **kwargs,
    ...
    if q_len == 1:
        ...
        # group_key_position -= group_key_position[group_key_position > 0].min()

        # group_key_position = group_key_position.clamp(0, cos.size(0) - 1)
